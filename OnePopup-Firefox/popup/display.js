...
function dispQR(url) {
    ...
    deets.innerHTML =
        "Please scan the QR Code with<br>your phone camera or<br>a QR code reader";
    // ---------------------------------------------------------
        ...
        for (tab of tabs) {
            ...
            if (tab.windowId != window) {
                ...
                newWindow.setAttribute(
                    "style",
                    "white-space: pre; margin: 7px auto 5px auto; height: auto; width: auto;"
                );
            ...
            let lastOpened = new Date(tab.lastAccessed);
            let lastOpenedDisplay = lastOpened.toLocaleString();
            let lastOpenedCalculate = lastOpened.getTime();
            let today = new Date().getTime();
            let noOfDaysSinceLastOpened = Math.round(
                (today - lastOpenedCalculate) / oneDay
            );
            // For button
            ...
            if (noOfDaysSinceLastOpened > 1) {
                tabLink.textContent = `${tabCounter}. ${tab.title ||
                    tab.id}\n\tLast accessed: ${noOfDaysSinceLastOpened} days ago`;
            } else {
                tabLink.textContent = `${tabCounter}. ${tab.title ||
                    tab.id}\n\tLast accessed: ${noOfDaysSinceLastOpened} day ago`;
            }
            ...
            tabLink.onclick = () =>
                redirectToTab(
                    ...
                    tabLink.getAttribute("data-id")
                );

            // If the tab was last opened more than 2 days ago, it will be highlighted
            // red in colour
            if (noOfDaysSinceLastOpened >= 5) {
                tabLink.classList.add("list-group-item-danger");
                // btn.classList.add('list-group-item-danger');
            } else if (
                1 < noOfDaysSinceLastOpened &&
                noOfDaysSinceLastOpened < 5
            ) {
                tabLink.classList.add("list-group-item-warning");
                // btn.classList.add('list-group-item-danger');
            }
            // If the tab is the active tab of the window, it will be highlighted via
            // this if statement
            if (tab.active) {
                tabLink.classList.add("list-group-item-primary");
